...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/fig_26_gitv_visualizer.jpg}\\[4pt]
\includegraphics[width=0.5\textwidth]{figures/fig_26_gitv_visualizer_board.jpg}
\caption{实时可视化 main 分支：commit → tree → blob 的引用关系全部来自磁盘原始字节；下方为同一时刻的板书：$V_2$ 只沿改动的 b.txt 到根的路径新建节点，含 10000 个文件的目录 d 仍指向 $V_1$ 已有的对象\vtag}
\end{figure}
