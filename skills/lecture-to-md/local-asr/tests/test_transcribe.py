...
覆盖 review #1 的硬要求：
- 本地 backend 把音频切成 ≤30s 的 chunk（TestChunking 直接断言分块边界）
...
需要外部依赖才跑得起来的用例会自动 skip：
    - TestBackendChunking    需要 X-ASR 模型 + sherpa_onnx 运行时
    - TestChunking           需要本地 backend + numpy（纯逻辑，不加载模型）
    - 其余纯逻辑用例无外部依赖
"""
...
BACKEND = os.path.join(SKILL_DIR, "scripts", "transcribe_x_asr.py")
...
class TestBackendChunking(unittest.TestCase):
    """在 mac / Linux 上跑真实的本地 backend，验证：
    1) 长音频被切成多个 chunk，每个 chunk 持续 ≤30s
    2) 拼接后 cues 的时间轴连续（无 overlap、无 gap >0.05s）

    依赖本地 backend + X-ASR 模型 + sherpa_onnx 运行时；缺任一则 skip。
    """
    ...
    def setUpClass(cls):
        ...
        if not os.path.isfile(BACKEND):
            raise unittest.SkipTest(f"backend not found: {BACKEND}")
    ...
    def _transcribe(self, wav_path: str) -> dict:
        ...
        with tempfile.TemporaryDirectory(prefix="asr-test-") as tmp:
            ...
            cmd = [
                sys.executable, BACKEND, wav_path,
                "--output", srt_path,
                "--report", report_path,
            ...
            if proc.returncode != 0:
                self.fail(f"backend failed: {proc.stderr[-500:]}")
...
class TestChunking(unittest.TestCase):
    ...
    直接导入本地 backend 的分块迭代器，喂一段合成 wav，断言每个产出的 chunk：
    ...
    def _iter_chunks(self, wav_path, **kw):
        ...
        import transcribe_x_asr as backend  # noqa: E402
        return list(backend.iter_wave_chunks(
...
class TestTokensToCues(unittest.TestCase):
    """本地 backend 的 token→cue：句边界 + 时间轴映射正确。"""

    def test_chinese_sentence_split(self):
        """以句末标点（。！？）拆句：一条多句输入 → 多条 cues。"""
        from transcribe_x_asr import tokens_to_cues

        # 两句话：「你今天。」、「这是测试。」
        tokens = [" 你", " 今", " 天", " 。", " 这", " 是", " 测试", " 。"]
        timestamps = [0.0, 0.2, 0.4, 0.5, 1.0, 1.2, 1.4, 1.6]
        cues = tokens_to_cues(
            tokens, timestamps, chunk_start=0.0, chunk_duration=2.0
        )
